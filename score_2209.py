...
#added numpy for updating annoatated image into DB
import numpy as np
...
def detect_potholescore_with_ID(image_id):
    ...
    ## added to test sending annotated img back ###
       # Generate annotated image using YOLOv8 results
        annotated_image = results[0].plot()  # Assuming this generates the image with annotations


       # Check if the annotated image exists (use isinstance for PIL Image or ensure it's not None)
        if annotated_image is not None:
            print("Annotated image has value")
            ...
            processed_image_base64 = base64.b64encode(buffered.getvalue()).decode('utf-8')
            print("Generated and encoded annotated image")
        else:
            print("Annotated image returned no value")
            processed_image_base64 = None  # Handle the case when there is no image
      ## end added to test sending annotated img back ###   
